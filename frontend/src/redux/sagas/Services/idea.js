...
export function RequestGetAllIdea() {
    let url = "http://localhost:8080/idea";
    let config = {
        params: {
...
export function requestGetIdea() {
    let url = "http://localhost:8080/idea/user";
    let config = {
        params: {
            email: localStorage.getItem('email'),
        },
...
export function RequestSingleGetIdea(id) {
    const api = `http://localhost:8080/idea/${id}`
    let config = {
...
export function RequestAddIdeas(obj) {
    ...
    const api = "http://localhost:8080/idea"
    let config = {
        params: {
            email: localStorage.getItem('email'),
        },
...
console.log("XYZ ", id);
let url = `http://localhost:8080/idea/${id}`;
let config = {
